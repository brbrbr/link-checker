...
class LinkQuery
{
    ...
    function get_links($params = null)
    {
        ...
        */
        ...
        if ($load_instances) {
            ...
            $all_instances = Utility::blc_get_instances($link_ids, $purpose, $params['load_containers'], $params['load_wrapped_objects']);
    ...
    function print_filter_menu($current = '')
    {
        ...
        foreach ($filters as $filter => $data) {
            ...
            $items[] = sprintf(
                "<li><a href='admin.php?page=blc_local&filter_id=%s' %s>%s</a> <span class='count'>(<span class='%s'>%s</span>)</span>",
                esc_attr($filter),
                $class,
                esc_html($data['name']),
                $number_class,
